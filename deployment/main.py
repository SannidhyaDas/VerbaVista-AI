...
from google.api_core.exceptions import ResourceExhausted  # Common for quota errors
...
def create_chunks(transcript):
    text_splitters= RecursiveCharacterTextSplitter(chunk_size=20000,chunk_overlap=1500)
    doc= text_splitters.create_documents([transcript])
    return doc


# function to create embedding and store it into an vector space.
def create_vector_store(docs, batch_size=1, max_retries=5, base_delay=60):
    embedding_model = GoogleGenerativeAIEmbeddings(model="models/gemini-embedding-001", transport="grpc")
    ...
    for i in range(0, len(texts), batch_size):
        ...
        while retries <= max_retries:
            try:
                ...
                # Check if it is a rate-limit/quota error (adjust exception types as needed)
                if hasattr(e, 'code') and e.code == 8:  # ResourceExhausted often has grpc code 8
                    wait_time = base_delay * (2 ** retries)  # exponential backoff
                    st.warning(f"Quota exceeded, retry {retries+1}/{max_retries}, sleeping {wait_time} seconds...")
            ...
            st.error(f"Failed to embed batch after {max_retries} retries due to quota limits.")
            return None
