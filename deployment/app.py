#------ Must Have first in this script[If not then chromadb will give error] ------
...
#-----------------------------------------------------------------------------------
#The above part is to fix the chromadb and sqlite3 issue in streamlit deployment
# (since Many managed services use older Python or OS images, leading to errors even if your 
# local system works fine. In those cases, the pysqlite3 override is often required and must be 
# in your requirements/config.)
...
with st.sidebar:
    ...
    language = st.text_input("Video Language Code", placeholder="e.g., en, hi, bn, fr", value="en")
            ...
            if task_option == "Chat with Video":
                with st.spinner("Step 2/3: Creating chunks and vector store...."):
                    ...
                    if vectorstore is None:
                        st.error("Vector store creation failed due to embedding quota limits. Please try again later.")
                    else:
...
if task_option == "Chat with Video":
    if "vector_store" not in st.session_state:
        st.info("Vector store is not initialized yet. Please start processing a video first.")
    elif st.session_state.vector_store is None:
        st.error("Vector store failed to initialize due to embedding quota limits. Please try again later.")
    else:
